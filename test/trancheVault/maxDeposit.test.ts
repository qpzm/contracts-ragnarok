import { ILenderVerifier__factory } from 'build/types'
import { expect } from 'chai'
import { deployMockContract } from 'ethereum-waffle'
import { structuredPortfolioFixture, structuredPortfolioLiveFixture } from 'fixtures/structuredPortfolioFixture'
import { setupFixtureLoader } from 'test/setup'
import { DAY } from 'utils/constants'
import { timeTravel } from 'utils/timeTravel'

describe('TrancheVault.maxDeposit', () => {
  const loadFixture = setupFixtureLoader()

  it('returns 0 when deposits are disabled in portfolio', async () => {
    const { equityTranche, wallet, equityTrancheData, setDepositAllowed } = await loadFixture(structuredPortfolioLiveFixture)
    await setDepositAllowed(equityTrancheData.depositController, false)
    expect(await equityTranche.maxDeposit(wallet.address)).to.eq(0)
  })

  it('returns 0 in Closed status', async () => {
    const { structuredPortfolio, equityTranche, wallet } = await loadFixture(structuredPortfolioLiveFixture)
    await structuredPortfolio.close()
    expect(await equityTranche.maxDeposit(wallet.address)).to.eq(0)
  })

  it('returns ceiling - totalAssets', async () => {
    const { equityTranche, wallet, depositToTranche, equityTrancheData } = await loadFixture(structuredPortfolioFixture)
    const depositAmount = 1000
    await depositToTranche(equityTranche, depositAmount)

    const ceiling = await equityTrancheData.depositController.ceiling()
    expect(await equityTranche.maxDeposit(wallet.address)).to.eq(ceiling.sub(depositAmount))
  })

  it('returns 0 for ceiling smaller than totalAssets', async () => {
    const { equityTranche, wallet, equityTrancheData, depositToTranche } = await loadFixture(structuredPortfolioFixture)
    const ceiling = await equityTrancheData.depositController.ceiling()
    await depositToTranche(equityTranche, ceiling)
    await equityTrancheData.depositController.setCeiling(ceiling.sub(1))
    expect(await equityTranche.maxDeposit(wallet.address)).to.eq(0)
  })

  it('returns 0 when lender not allowed by lender verifier', async () => {
    const { equityTranche, wallet, equityTrancheData: { depositController } } = await loadFixture(structuredPortfolioFixture)
    const mockLenderVerifier = await deployMockContract(wallet, ILenderVerifier__factory.abi)
    await mockLenderVerifier.mock.isAllowed.returns(false)
    await depositController.setLenderVerifier(mockLenderVerifier.address)
    expect(await equityTranche.maxDeposit(wallet.address)).to.eq(0)
  })

  it('returns 0 when tranche is drained', async () => {
    const { seniorTranche, structuredPortfolio, addAndFundLoan, getLoan, protocolConfig, wallet } = await loadFixture(structuredPortfolioLiveFixture)
    await protocolConfig.setDefaultProtocolFeeRate(100)
    const defaultedLoanAmount = (await structuredPortfolio.totalAssets()).sub(1)
    const loanId = await addAndFundLoan(getLoan({
      principal: defaultedLoanAmount,
      periodDuration: DAY,
      periodCount: 1,
      gracePeriod: 0,
    }))
    await timeTravel(3 * DAY)
    await structuredPortfolio.markLoanAsDefaulted(loanId)
    expect(await seniorTranche.maxDeposit(wallet.address)).to.equal(0)
  })
})
